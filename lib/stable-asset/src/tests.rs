...
#[test]
fn modify_recipients_successful() {
	new_test_ext().execute_with(|| {
		let pool_tokens = create_pool();
		match pool_tokens {
			(coin0, coin1, pool_asset, swap_id) => {
				assert_ok!(StableAsset::modify_recipients(
					Origin::signed(1),
					0,
					Some(100),
					Some(200),
...
#[test]
fn modify_asset_successful() {
	new_test_ext().execute_with(|| {
		let pool_tokens = create_pool();
		match pool_tokens {
			(coin0, coin1, _pool_asset, swap_id) => {
				assert_ok!(StableAsset::modify_asset(Origin::signed(1), 0, 100,));
				assert_eq!(
					StableAsset::pools(0),
					Some(StableAssetPoolInfo {
						pool_asset: 100,
