	...
	new_test_ext().execute_with(|| {
		...
		match pool_tokens {
			(coin0, coin1, pool_asset, swap_id) => {
				...
				if let Event::StableAsset(crate::pallet::Event::Minted{who: _, pool_id: _, amount: mint_amount, input_asset: _, fee: fee_amount}) = last_event()
				{
	...
	new_test_ext().execute_with(|| {
		...
		match pool_tokens {
			(coin0, coin1, pool_asset, swap_id) => {
				...
				if let Event::StableAsset(crate::pallet::Event::TokenSwapped{swapper: _, pool_id: _, input_asset: _, output_asset: _, input_amount: dx, output_amount: dy}) = last_event() {
	...
	new_test_ext().execute_with(|| {
		...
		match pool_tokens {
			(coin0, coin1, pool_asset, swap_id) => {
				...
				if let Event::StableAsset(crate::pallet::Event::Redeemed{redeemer: _, pool_id: _, amount, input_amount: amounts, fee: fee_amount}) =
					last_event()
				{
